...
{
    public CustomScreenManager cardManager;
   ...
   void Init()
    ...
    public void UpdateDeckView()
    {
        ...
        if (cardManager.GetChosenCards() != null && cardManager.GetChosenCards().Count > 0)
        {
            ...
            cardName.text = cardManager.GetChosenCards()[0].battleCard.cardName;
            cardImage.sprite = cardManager.GetChosenCards()[0].battleCard.icon;
            cardDamage.text = cardManager.GetChosenCards()[0].battleCard.damage.ToString();

            // Instantiate new cardGradeBox objects for the remaining cards, starting from the back
            for (int i = cardManager.GetChosenCards().Count - 1; i > 0; i--)
            {
                var newCard = Instantiate(cardGradeBox, cardGradeTracker);
                newCard.GetComponent<Image>().color = cardManager.GetChosenCards()[i].battleCard.colorIndicator;
            }
        }
        else
        {
